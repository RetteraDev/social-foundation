      ...
      <Card>
        ...
        <CardContent className="space-y-4 sm:space-y-6">
          {projectsData.map((project) => {
            const shouldShowEllipsis = project.results && project.results.length > 0;
            const maxLength = window.innerWidth < 640 ? 150 : 300;
            const previewText = shouldShowEllipsis && project.results.length > maxLength
              ? `${project.results.substring(0, maxLength)}...`
              : project.results || '';
